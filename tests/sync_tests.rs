...
fn test_sync_back<'a, T, E, P>(
    ...
    P: AsRef<Path>,
{
    let te = TestEnv::with_tree_paths(srcs.iter().cloned())
        .with_dsts_paths(dsts.iter().cloned())
...
fn test_apply_to_dst(
    ...
    // last_dsts.iter().filter(|s| target_dsts.iter().any(|d| s.starts_with(d)));
}

#[rstest]
#[case(&DEFAULT_PATHS, &[], &[], &[])]
#[case(&DEFAULT_PATHS, &DEFAULT_PATHS, &[&DEFAULT_PATHS.map(|v| v[0])[..], &SIMPLE.map(|v| v[0])].concat(), &["home", "boot", "etc/openresty"])]
#[case(&DEFAULT_PATHS, &DEFAULT_PATHS, &DEFAULT_PATHS.iter().flat_map(|v| v.iter()).copied().collect_vec(), &["home", "boot", "etc/openresty"])]
fn test_clean_dst(
    #[case] srcs: &[&[&str]],
    #[case] dsts: &[&[&str]],
    #[case] last_dsts: &[&str],
    ...
    // let v = DEFAULT_PATHS.iter().flat_map(|v| v.iter()).copied().collect_vec();
    let te = TestEnv::new(srcs.iter().copied())
        .with_dsts(dsts.iter().copied())
        .with_last_dsts(last_dsts)
        .with_copier(CopierBuilder::default().build().unwrap());
